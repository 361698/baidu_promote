...
- 先读 `route.md`。`route.md` 不是选择最短路径，短不短无所谓；它索引的是已经沉淀的、能够运行的常用命令，例如知识库搜索、企业内搜、内搜详情、会议搜索、周报搜索、OKR 搜索、搜人和搜群；如果里面有可直接复用的命令，优先按它执行。
- 如果运行中发现环境、路径、依赖、认证或参数问题，并形成可复用修正，一定要更新 `route.md`。
- 搜群拿到“群名+gid（群号）”时，追加/更新到 `knowledge-fetch` skill 的 `group-directory.yaml`（群名↔群号缓存，只存群名和群号），方便用户后续直接说群名取号。
- 如果用户指出内容性质、格式、业务风格、写作方式、结果筛选、摘要风格、文字风格、长度、信息编造等可沉淀问题，一定要更新 `experiment.md`；`experiment.md` 负责沉淀内容和呈现经验，`route.md` 只负责沉淀可运行命令；只写 mock 示例，不写真实人名、群名、链接、token、AK/SK。
...
6. 当搜索公开知识或实时公开信息时，使用 `realtime-search`
7. 其他场景 → 使用本 skill 进行搜索和详情获取
