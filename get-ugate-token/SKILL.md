...
- 先读 `route.md`，按其中命令选择最短可运行路径。
- 运行和排障时优先参考 `experiment.md` 的可复用经验。
- 如果运行中发现环境、路径、依赖、认证或参数问题，并形成可复用修正，请更新 `route.md` 的一键命令。
- 如果用户指出引导话术、缓存策略、隐私边界等可沉淀问题，请更新 `experiment.md`；只写 mock 示例，不写真实 token、AK/SK 或用户私密信息。
