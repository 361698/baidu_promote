...
- 如流群号。只有安装或验收 `knowledge-fetch` 时需要。问群号时，先告诉用户必须把 dodo 机器人拉进群：打开群聊，点击右侧人员按钮，点击添加成员，在搜索框内搜索 dodo 并拉到群里，然后点击群聊名称，复制群号发过来。如果用户没有可测群号，先跳过群聊拉取验收，并在最后说明。
...
5. 如果要验收群聊拉取，请先把 dodo 机器人拉进群：打开群聊，点击右侧人员按钮，点击添加成员，在搜索框内搜索 dodo 并拉到群里；然后点击群聊名称，复制群号发给我。
...
- 在用户个人知识库创建一篇名为 `hello world` 的欢迎文档，介绍这些 skill 的能力。
- 开头先感谢 onetool 技能 `https://console.cloud.baidu-int.com/onetool/skills/1638`、`https://console.cloud.baidu-int.com/onetool/skills/154`、`https://console.cloud.baidu-int.com/onetool/skills/1433` 和 dodo 团队。
- 邀请用户加入如流群聊 `13060869`（该群暂未开启搜索，请联系潘英培）解锁更多玩法。
- 推荐一首闽南、粤语或国语老歌，并通过互联网搜索真实图片，放进表格里介绍这首老歌的历史。
- 发布后读回，确认标题、正文、文档链接、表格和真实图片都存在。

### `enterprise-search`

- 检索刚刚生成的 `hello world` 欢迎文档，确认能搜到并返回结果。

### `knowledge-fetch`

- 使用开头收集到的可测试如流群号拉取完整群聊信息，并把本地结果地址返回给用户；如果没有群号，跳过并说明缺少群号。
- 在用户个人知识库主页再创建一篇文档，展示这个群里不同用户过去一周的主要工作罗列。
