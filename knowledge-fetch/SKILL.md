...
- 拉取和落盘数据时优先参考 `experiment.md` 的可复用经验。
- 如果运行中发现环境、路径、依赖、认证、配置字段或 source 参数问题，并形成可复用修正，请更新 `route.md` 的一键命令。
- 如果用户指出输出结构、摘要方式、取数范围、信息编造等可沉淀问题，请更新 `experiment.md`；只写 mock 示例，不写真实群名、仓库名、链接、token、AK/SK。
