...
本文件只沉淀可运行命令，不承载内容结构和写作约束。写入、创建、编辑、覆盖、追加或发布 KU 文档正文前，必须先读 `SKILL.md` 首要提示和 `experiment.md`，再使用本文件选命令。读回发现 Markdown 进入标题隐藏内容、正文只剩标题或空卡片时，不是正常成功态，必须用编辑器 JSON `cover` 重写正文。
